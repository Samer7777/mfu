...
(function ($, window, document, undefined) {
  ...
  var Pojo_Accessibility_App = {
    ...
    bindEvents: function () {
      ...
      $(document).on("keyup", function (event) {
        ...
        if (
          TAB_KEY !== event.which ||
          !$self.cache.$btnToolbarToggle.is(":focus")
        ) {
          return;
        }
    ...
    getButtonByAction: function (action) {
      return this.cache.$toolbarToolsLinks.filter(
        "." + this.settings.buttonsClassPrefix + action
      );
    },
    ...
    actions: {
      ...
      resize: function (action, deactivate) {
        ...
        if (
          "resize-plus" === action &&
          this.settings.maxFontSize > oldFontSize
        ) {
        ...
        if (
          "resize-minus" === action &&
          this.settings.minFontSize < oldFontSize
        ) {
          this.variables.currentFontSize -= 10;
        }
        ...
        this.cache.$body.removeClass(
          this.settings.bodyFontClassPrefix + oldFontSize
        );
        ...
        if (isPlusActive) {
          this.cache.$body.addClass(
            this.settings.bodyFontClassPrefix + this.variables.currentFontSize
          );
        }
      ...
      schema: function (action, deactivate) {
        ...
        if (currentSchema) {
          this.cache.$body.removeClass(
            this.settings.bodyClassPrefix + currentSchema
          );
    ...
    reset: function () {
      for (var action in this.activeActions) {
        if (
          this.activeActions.hasOwnProperty(action) &&
  ...
  $(document).ready(function ($) {
    ...
    var grayscaleclicked = false;
    $(".pojo-a11y-btn-grayscale").click(function () {
      if (!grayscaleclicked) {
        grayscaleclicked = true;
        $("body").removeClass("pojo-a11y-grayscale");
        $(".for-grayscale-jump-fix").addClass("pojo-a11y-grayscale");
        $("#pojo-a11y-toolbar .pojo-a11y-toolbar-toggle a").css(
          "background-color",
          "gray"
        );
        //Fancybox Need
        $("#pojo-a11y-toolbar.pojo-a11y-toolbar-left").css("z-index", "2500");
        $("#pojo-a11y-toolbar").css("z-index", "2500");
      } else {
        grayscaleclicked = false;
        $(".for-grayscale-jump-fix").removeClass("pojo-a11y-grayscale");
        $("#pojo-a11y-toolbar .pojo-a11y-toolbar-toggle a").css(
          "background-color",
          "#83AD5B"
        );
